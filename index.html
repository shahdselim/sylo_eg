...
<head>
  ...
  <style>
    ...
    @keyframes pulse {
        ...
        50% { transform: scale(1.01); box-shadow: 0 6px 20px rgba(0, 0, 0, 0.2); }
...
<body class="bg-white font-['Montserrat'] flex flex-col min-h-screen">
  ...
  <section id="home" class="min-h-screen flex flex-col items-center justify-center bg-white text-black py-20">
    <div class="container mx-auto px-4 text-center z-10 flex-grow flex flex-col justify-center">
    ...
    <div class="w-full max-w-5xl mt-12 px-4">
        <div class="ad-banner">
            ...
            <p>Buy 2 Apple Watch Bands, Get 1 FREE! Mix & Match Your Style.</p>
            <p class="mt-2 text-sm text-gray-700 font-bold">Use code: SYLOFREEBAND at checkout!</p>
        </div>
    </div>
  </section>
  ...
  <section id="products" class="container mx-auto py-16 hidden">
    ...
    <div class="w-full max-w-5xl mx-auto my-8 px-4">
        <div class="ad-banner">
            ...
            <p>Upgrade Your Look: 20% Off All Leather Bands This Week!</p>
            <p class="mt-2 text-sm text-gray-700 font-bold">No code needed. Discount applied automatically.</p>
        </div>
    </div>

    <div id="product-list" class="product-grid">
      </div>
    <p id="no-products-found" class="text-center text-gray-600 mt-8 hidden">No products match your search or filter.</p>
  </section>
  ...
  <section id="checkout" class="container mx-auto py-16 hidden">
    ...
    <p id="checkout-summary-total" class="text-xl font-semibold text-black mt-6 text-right"></p>

    <div class="max-w-lg mx-auto mt-8 p-6 bg-white rounded-lg shadow-md">
      <h3 class="text-2xl font-bold text-black mb-6">Shipping Information</h3>
      <div class="space-y-4">
        <div>
      ...
      <h3 class="text-2xl font-bold text-black mt-10 mb-4">Promo Code</h3>
      <div class="flex space-x-2 mb-6">
        <input type="text" id="promo-code-input" class="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-black focus:border-transparent" placeholder="Enter promo code">
        <button id="apply-promo-btn" class="bg-gray-700 text-white px-4 py-2 rounded-lg hover:bg-black transition">Apply</button>
      </div>

      <button id="place-order-btn" class="bg-black text-white px-6 py-3 rounded-full hover:bg-gray-800 transition mt-8 w-full">Place Order</button>
    </div>
  </section>
...
// Replace the old signupForm.addEventListener with this new one
signupForm.addEventListener('submit', function(event) {
    event.preventDefault(); // Prevent default form submission

    // Client-side validation (already exists, keep this part)
    let isValid = true;
    ...
    // Prepare form data for sending to PHP
    const formData = new FormData();
    formData.append('name', signupNameInput.value.trim());
    formData.append('email', signupEmailInput.value.trim());
    formData.append('password', signupPasswordInput.value); // Don't trim password here
    ...
    .then(data => {
        showCustomAlert(data.message); // Show message from PHP
        if (data.success) {
            signupForm.reset();
...
// Replace the old loginForm.addEventListener with this new one
loginForm.addEventListener('submit', function(event) {
    ...
    // Prepare form data for sending to PHP
    const formData = new FormData();
    formData.append('email', loginEmailInput.value.trim());
    ...
    .then(response => {
        if (!response.ok) {
            // If the response is not OK, try to parse error message from JSON
            return response.json().then(errorData => {
                throw new Error(errorData.message || `HTTP error! status: ${response.status}`);
            });
        }
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // e.g., change "Sign Up" button to "Welcome, [User Name]"
...
contactForm.addEventListener('submit', function(event) {
    ...
    if (!isValid) {
        ...
        // This is a client-side simulation for the success message.
        // In a real application, you would handle the Formspree response.
        setTimeout(() => {
            showCustomAlert('Thank you for your message! We will get back to you soon.');
            contactForm.reset();
        }, 500); // Give Formspree a moment to process
    }
});
...
// Replace the old supportForm.addEventListener with this new one
supportForm.addEventListener('submit', function(event) {
    ...
    // Prepare form data for sending to PHP
    const formData = new FormData();
    formData.append('name', supportNameInput.value.trim());
    formData.append('email', supportEmailInput.value.trim());
    formData.append('message', supportMessageInput.value.trim());

    // Send data to submit_support.php
...
document.addEventListener('DOMContentLoaded', () => {
    updateCart(); // Initialize cart count and display on page load
    showSection('home'); // Show home section by default

    // Ensure overlays and custom alert start hidden
    leftSlideMenuOverlay.classList.remove('open');
    searchOverlay.classList.remove('open');
    cartOverlay.classList.remove('open');
    signupModal.classList.remove('open');
    ...
    // Initial display of products (e.g., all products)
    displayProducts('all');
});
